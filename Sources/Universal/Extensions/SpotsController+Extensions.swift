...
public extension SpotsController {
  ...
  func ui<T: View>(_ includeElement: (Item) -> Bool) -> T? {
  ...
  func itemModel<T>(_ includeElement: (Item) -> Bool) -> T? {
    for component in components {
  ...
  func filter(components includeElement: (Component) -> Bool) -> [Component] {
  ...
  func filter(items includeElement: (Item) -> Bool) -> [(component: Component, items: [Item])] {
  ...
  func cache(_ amountOfItems: Int? = nil) {
    #if DEVMODE
